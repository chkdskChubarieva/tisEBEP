...
export const useDrag = () => {
  ...
  const handleDragEnd = (onDrop: (source: string, destination: string, sourceIndex: number, destinationIndex: number) => void) => {
    if (!isDragging || !draggedItem.current) return;
    ...
    const taskIndex = parseInt(draggedItem.current.getAttribute('data-task-index') || '0', 10);
    ...
    dropZones.forEach((zone) => {
      ...
      if (
        ...
        if (sourceColumnId && destinationColumnId) {
          onDrop(sourceColumnId, destinationColumnId, taskIndex, destinationIndex);
        }
      }
    });
