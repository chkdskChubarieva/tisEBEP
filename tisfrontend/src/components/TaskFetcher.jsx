...
const TaskFetcher = () => {
  ...
  const fetchTasks = async () => {
    ...
    try {
      const response = await axios.get(`http://localhost:8000/api/v1/user-story/${userStoryId}/tasks`);
      console.log(response.data)
      if (response.data && Array.isArray(response.data.tareas)) {
        setTasks(response.data.tareas);
      } else {
        setError('No se pudieron obtener las tareas. La estructura de la respuesta es incorrecta.');
      }
    } catch (error) {
      setError('No se pudo obtener las tareas para el ID de historia de usuario proporcionado.');
    }
  };
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '500px', margin: 'auto', fontFamily: 'Arial, sans-serif' }}>
      ...
      <ul style={{ listStyleType: 'none', padding: 0 }}>
        {tasks.length > 0 ? (
          tasks.map((task) => (
            <li key={task.ID_tarea} style={{ marginBottom: '10px', padding: '10px', border: '1px solid #ddd', borderRadius: '5px' }}>
              <strong>Task #{task.nro_tarea}</strong>
              <p><strong>Contenido:</strong> {task.contenido_tarea}</p>
              <p><strong>Estado:</strong> {task.estado}</p>
              <p><strong>Estimación:</strong> {task.estimacion} horas</p>
            </li>
          ))
        ) : (
