...
const ProductBacklog = () => {
  ...
  useEffect(() => {
    ...
    const fetchCompanyId = async () => {
      try {
        console.log(userId)
        const response = await axios.get(`http://localhost:8000/api/v1/estudiantes/usuario/${userId}`);
    ...
    <>
      ...
      <div className="w-full px-10 py-6 product-backlog">
        ...
        <div className="flex flex-col gap-8 mt-5 lg:flex lg:flex-row">
          <section className="flex-1 order-last border-2 rounded-md tabla-backlog border-neutral-300 lg:order-first">
            <div className="flex justify-between px-6 py-3 font-semibold border-b-2 border-neutral-300 text-primary-800">
              <span>Historia de usuario</span>
              <span># Sprint</span>
            </div>
          ...
          <section className="border-2 rounded-md lg:w-64 h-fit border-neutral-300">
            <div className="flex justify-between px-6 py-3 font-semibold border-b-2 border-neutral-300 text-primary-800">
              <span>Sprints</span>
              <button
                type="button"
